...
9) Чтобы исправить это, сделай следующее:
    + запусти TES5Edit через MO2, выбери один Update.esm, дождись конца загрузки (Background Loader: finished).
    ...
    + после всего в папке Overwrite MO2 у тебя могут возникнуть чистые плагины DLC и папка с бэкапами. Создай из них мод (ПКМ по Overwrite ➔ Create Mod), назови "Cleaned DLC" или как-то так. Помести его первым, а когда активируешь - не забудь снова про порядок загрузки.
